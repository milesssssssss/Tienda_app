...
const ModalEditarCantidad = ({setModalVisible, modalVisible, idDetalle, idProducto, setCantidadProductoCarrito, cantidadProductoCarrito, getDetalleCarrito}) => {
  ...
  const handleUpdateDetalleCarrito = async () => {
    try {
      ...
      formData.append('idProducto', idProducto);  // Asegúrate de tener idProducto disponible
      ...
      console.log(data); // Agrega un log para revisar la respuesta
  
      if (data.status) {
        Alert.alert('Se actualizó el detalle del producto');
        getDetalleCarrito();
      } else {
        // Maneja el caso cuando 'data.error' es un objeto
        const errorMessage = typeof data.error === 'string' ? data.error : JSON.stringify(data.error);
        Alert.alert('Error al editar detalle carrito', errorMessage);
      }
      setModalVisible(false);
    } catch (error) {
      Alert.alert("Error al editar carrito", error.message);
      setModalVisible(false);
    }
  };
